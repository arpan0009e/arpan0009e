# 👋 Hi, I'm Arpan Mondal

🎓 B.Tech (AI & ML) Student | 🤖 Aspiring AI/ML Engineer  
📍 Jaipur, India  

---

## 🚀 About Me
- Passionate about **Machine Learning, Deep Learning & Computer Vision**
- Hands-on experience with **Python, PyTorch, Scikit-learn**
- Love building **end-to-end ML projects** and deploying them
- Actively seeking **Internship / Entry-level AI-ML roles**

---

## 🛠️ Skills
- **Languages:** Python, C, C++
- **ML/DL:** Regression, Classification, CNN, PyTorch
- **Libraries:** NumPy, Pandas, Matplotlib, Scikit-learn
- **Tools:** Git, GitHub, VS Code, Jupyter Notebook
- **Databases:** MySQL

---

## 📌 Featured Projects
### 🐶 Cat vs Dog Image Classifier
🔹 CNN-based image classification using PyTorch  
🔹 Accuracy: 92%  
🔹 Includes training, evaluation & deployment-ready structure  
👉 [View Project](https://github.com/arpan0009e/cat-dog-image-classifier)

### 🍕 Pizza Price Prediction
🔹 Regression-based ML model  
🔹 Data preprocessing + visualization  
👉 [View Project](https://github.com/arpan0009e/Pizza_Price_Prediction)

### 🚚 Food Delivery Time Prediction
🔹 Real-world dataset  
🔹 Feature engineering + model evaluation  
👉 [View Project](https://github.com/arpan0009e/Food_Delivery_Time_Prediction)

---

## 📊 GitHub Stats
![Arpan's GitHub stats](https://github-readme-stats.vercel.app/api?username=arpan0009e&show_icons=true&theme=tokyonight)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=arpan0009e&layout=compact&theme=tokyonight)

---

## 📫 Connect With Me
- 💼 LinkedIn: *(add your LinkedIn link)*
- 📧 Email: *(professional email)*

⭐ *I love learning and building real-world AI solutions*
